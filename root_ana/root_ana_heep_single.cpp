...
#include <iostream>
#include "root_ana_heep_single.h"
...
//  	eff_setting = new ReadFile::eff_pro[set_runs];
	...
	/// Mod
 	...
 	ana_dummy 	       = new Analysis_heep_single*[set_runs];
 	ana_target         = new Analysis_heep_single*[set_runs];
 	ana_sim            = new Analysis_heep_single*[set_runs];
// 
...
TString Dummy_Name(int num_itt) {
	...
	if (int(kin_setting.thpqset[num_itt]*1000) == 0) {
		...
		dir_str.Form("yields.heep_%i_%i_0000.dummy", int(round(kin_setting.Q2set[num_itt]*100)), int(kin_setting.epsset[num_itt]*100));	

	} else {

		dir_str.Form("yields.heep_%i_%i_%i.dummy", int(round(kin_setting.Q2set[num_itt]*100)), int(kin_setting.epsset[num_itt]*100), int(kin_setting.thpqset[num_itt]*1000));	
...
TString Target_Name(int num_itt) {
	...
	if (int(kin_setting.thpqset[num_itt]*1000) == 0) {
		...
		dir_str.Form("yields.heep_%i_%i_0000.target", int(round(kin_setting.Q2set[num_itt]*100)), int(kin_setting.epsset[num_itt]*100));	

	} else {

		dir_str.Form("yields.heep_%i_%i_%i.target", int(round(kin_setting.Q2set[num_itt]*100)), int(kin_setting.epsset[num_itt]*100), int(kin_setting.thpqset[num_itt]*1000));	
...
TString Sim_Name(int num_itt) {
	...
	dir_str.Form("yields.heep_%i_%i_0000.sim", int(round(kin_setting.Q2set[num_itt]*100)), int(kin_setting.epsset[num_itt]*100));	
...
int Return_Setting_dummy(int list_itt, int run_itt, int set_itt) {
	...
	if (int(eff_setting_dummy[set_itt].thpqset[run_itt]*1000) == 0) {
		...
		run_setting_name.Form( "yields.heep_" + Q2_vec_mod[list_itt] + "_0000.dummy", int(eff_setting_dummy[set_itt].thpqset[run_itt]*1000)); 

	} else {

		run_setting_name.Form( "yields.heep_" + Q2_vec_mod[list_itt] + "_%i.dummy", int(eff_setting_dummy[set_itt].thpqset[run_itt]*1000));
...
//  	cout << "2222222222 " << endl;
	...
	if (int(eff_setting_target[set_itt].thpqset[run_itt]*1000) == 0) {
		...
		run_setting_name.Form( "yields.heep_" + Q2_vec_mod[list_itt] + "_0000.target", int(eff_setting_target[set_itt].thpqset[run_itt]*1000)); 

	} else {

		run_setting_name.Form( "yields.heep_" + Q2_vec_mod[list_itt] + "_%i.target", int(eff_setting_target[set_itt].thpqset[run_itt]*1000));
...
int Return_Setting(int list_itt, int run_itt, int set_itt) {
	...
	if (int(eff_setting[set_itt].thpqset[run_itt]*1000) == 0) {
		...
		run_setting_name.Form( "yields.heep_" + Q2_vec[list_itt] + "_0000.dummy", int(eff_setting[set_itt].thpqset[run_itt]*1000)); 

	} else {

		run_setting_name.Form( "yields.heep_" + Q2_vec[list_itt] + "_%i.dummy", int(eff_setting[set_itt].thpqset[run_itt]*1000));
...
//			list_name = list_dir + "list.dummy_" + List_Name(i) + "_plus";
			list_name = list_dir + "list.dummy_" + List_Name(i) + "_heep";
	...
	//			ana_dummy[ii] = new Analysis(rf->eff_pro1);
				ana_dummy[ii] = new Analysis_heep_single(rf->eff_pro1);
...
void Target_Load() {
	...
	for (Int_t i = 0; i < set_runs; i++) {
		...
		if (kin_setting.polset[i] > 0) {
			...
			list_name = list_dir + "list." + List_Name(i) + "_heep";
			...
			if (!rf->File_exist(list_name) && rf->File_empty(list_name)) {
				...
				ana_target[iiii] = new Analysis_heep_single(rf->eff_pro1);
...
// ////				ana_dummy[i]->Run_by_Run_Analysis(setting_run_list_dummy[i][ii]);
				ana_dummy[i]->Heep_anna_single(setting_run_list_dummy[i][ii]);
//				cout << ii << endl;
// 
 			}
// 
...
//				ana_target[i]->Run_by_Run_Analysis(setting_run_list_target[i][ii]);
				ana_target[i]->Heep_anna_single(setting_run_list_target[i][ii]);
...
void Sim_Load() {
	...
	for (Int_t i = 0; i < set_runs; i++) {
		...
		if (kin_setting.polset[i] > 0) {
			...
			list_name = List_Name(i) + "_heep";

			sim_vec.push_back(list_name);
				
			ana_sim[i] = new Analysis_heep_single();
...
//		output_file_sim[i]->ls();
		...
		ana_sim[i]->is_run_dummy = false;		
		ana_sim[i]->Heep_sim_anna_single(i);

// 
// 
// 
// 		cout << lists_vec_itt[i] << endl;
// 		
// // 		if (setting_run_list_target[i].size() != 0 ) {
// // 
// // //			cout << "kissy kissy" << endl;
...
// // 				ana_target[i]->is_run_dummy = false;
// // //				ana_target[i]->Run_by_Run_Analysis(setting_run_list_target[i][ii]);
// // 				ana_target[i]->Heep_anna(setting_run_list_target[i][ii]);
// // 
// // 			}
// // 
// // 			cout << "******************" << endl;
